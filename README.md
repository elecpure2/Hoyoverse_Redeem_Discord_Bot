...
- **데이터 소스 이전**: hakush.in 폐쇄 → [`nanoka.cc`](https://nanoka.cc)로 전환, 캐릭터/무기/성유물 조회 복원
- **스킬/무기 설명 정상화**: 데미지 배율을 기본 표시 레벨(원신·스타레일 Lv.10) 기준으로 표시, 무기는 1재련 기준. `{LINK}`·색상 태그 등 특수문자 정리
- **ZZZ 스킬 설명 복구** + **6돌파(M6) 일러스트** 표시
- **신캐 목록 `[Fix]` 태그**: 기존 캐릭터가 데이터/밸런스 수정으로 재등장하면 `[Fix]` 표시 (진짜 신규 캐릭터는 태그 없음)
- **기원 시뮬레이터**: 산드로네 픽업 + 100회 결과를 동그라미 그리드로 시각화
- **오늘의 운세** 캐릭터 추가 + **카드형 목록 디자인**
- **명령어 정리**: `뽑기 → 운세`, `하쿠신테스트 → 업데이트상태` 등 / 도움말 관리자 전용 섹션 분리
- 명조(WuWa) 코드 알림 비활성 (실시간 제공 사이트 없음)
